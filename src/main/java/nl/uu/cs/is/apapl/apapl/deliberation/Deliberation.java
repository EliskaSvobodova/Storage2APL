...
public class Deliberation implements Cloneable
{
	...
	public synchronized void step(APLModule module)
	{
		...
		synchronized(module)
		{
			APLBenchmarker.startTiming(module, APLBenchmarkParam.DELIB_STEP);
			result = currentStep.execute(module);
			APLBenchmarker.stopTiming(module, APLBenchmarkParam.DELIB_STEP);
	...
	public void addStep(DeliberationStep step) {
		cycle.add(step);
	}
