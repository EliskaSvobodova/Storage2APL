...
public class ConditionalPlan extends Plan
{
	...
	public String pp(int t)
	{
		...
		+		Base.tabs(t) + "\t"+thenPlan.pp(t+1)
		+		(elsePlan.isEmpty()?"": "\n"+Base.tabs(t) + "else\n" + Base.tabs(t) + "\t"+elsePlan.pp(t+1));
	}
	
	public String toRTF(int t)
	{
		return "\\cf2 if\\cf0 \\tab "+condition.toRTF(true)+"\\par\n"
		+		Base.rtftabs(t) + "\\cf2 \\cf0 \\tab "+thenPlan.toRTF(t+1)
		+		(elsePlan.isEmpty()?"": "\\par\n"+Base.rtftabs(t) + "\\cf2 else\n\\cf0 \\tab "+elsePlan.toRTF(t+1));
	}
	
	public String toRTF()
	{
		return "\\cf2 if\\cf0  "+condition.toRTF(true)+" "
		+		"\\cf2 \\cf0  "+thenPlan.toRTF()
		+		(elsePlan.isEmpty()?"": "\\cf2 else\n\\cf0  "+elsePlan.toRTF());
	}
